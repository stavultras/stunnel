...
NOEXPORT void direct_client_late(CLI *c) {
    ...
    if(c->opt->protocol_host) { /* explicit destination */
        char *req=str_printf("%c%c%s", 0x01, (int)strlen(c->opt->protocol_host), c->opt->protocol_host);
        ...
        switch(addr.sa.sa_family) {
            case AF_INET:
                req_v4.type = 0x02;
                memcpy(&req_v4.addr, &addr.in.sin_addr, 4);
                memcpy(&req_v4.port, &addr.in.sin_port, 2);
                s_log(LOG_INFO, "Sending direct IPv4 address");
                s_ssl_write(c, &req_v4, sizeof req_v4);
            break;
#ifdef USE_IPv6
            case AF_INET6:
                req_v6.type = 0x03;
                memcpy(&req_v6.addr, &addr.in6.sin6_addr, 16);
                memcpy(&req_v6.port, &addr.in6.sin6_port, 2);
                s_log(LOG_INFO, "Sending direct IPv6 address");
                s_ssl_write(c, &req_v6, sizeof req_v6);
            break;
#endif
            default:
                s_log(LOG_ERR, "Unsupported address type 0x%02x", addr.sa.sa_family);
...
NOEXPORT void direct_server_middle(CLI *c) {
    uint8_t host_len, type;
    uint8_t close_connection=0;
    ...
    switch (type) {
        ...
        case 0x02: // ipv4
            ...
            c->connect_addr.addr=str_alloc(sizeof(SOCKADDR_UNION));
            c->connect_addr.addr[0].in.sin_family=AF_INET;
            s_ssl_read(c, &resp_v4, 4+2);
            memcpy(&c->connect_addr.addr[0].in.sin_addr, &resp_v4.addr, 4);
            memcpy(&c->connect_addr.addr[0].in.sin_port, &resp_v4.port, 2);
        ...
        case 0x03: //ipv6
            ...
            c->connect_addr.addr=str_alloc(sizeof(SOCKADDR_UNION));
            c->connect_addr.addr[0].in6.sin6_family=AF_INET6;
            s_ssl_read(c, &resp_v6, 16+2);
            memcpy(&c->connect_addr.addr[0].in6.sin6_addr, &resp_v6.addr, 16);
            memcpy(&c->connect_addr.addr[0].in6.sin6_port, &resp_v6.port, 2);
...
NOEXPORT void direct_socks5_client_late(CLI *c) {
    ...
    uint8_t i, nmethods, *methods;

    SOCKS5_UNION socks;
    uint8_t host_len, close_connection = 1;
    char *host_name, *port_name;
    u_short port_number;
    ...
    if(socks.req.ver!=0x05) {
        ...
        if(socks.req.atyp==0x01) { /* IP v4 address */
            ...
            req_v4.type = 0x02;
            memcpy(&req_v4.addr, &socks.v4.addr, 4);
            memcpy(&req_v4.port, &socks.v4.port, 2);
            ...
            char *req=str_printf("%c%c%s:%s", 0x01, (int)(strlen(host_name) + 1 + strlen(port_name)), host_name, port_name);

            s_log(LOG_INFO, "SOCKS5 DOMAIN received \"%s:%s\"", host_name, port_name);
            ...
            req_v6.type = 0x03;
            memcpy(&req_v6.addr, &socks.v6.addr, 16);
            memcpy(&req_v6.port, &socks.v6.port, 2);
